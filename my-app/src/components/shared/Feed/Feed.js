...
export function Feed({type, amount, user_id, regexp}) {
    ...
    return (
        ...
        <div>
          ...
          {data && data.map(({ uuid, author, text_prompt, render_type, duration, userdets, timestamp, dominant_color, thumbnails}) => (
              <Skeleton borderRadius="lg" isLoaded={!loading}>
                <Preview width="384px" height="384px" thumbnails={thumbnails} dominant_color={dominant_color?dominant_color:[0,0,0]} userdets={userdets} timestamp={timestamp} key={uuid} uuid={uuid} text_prompt={text_prompt} render_type={render_type} duration={duration}/>
              </Skeleton>
            ))}
          </SimpleGrid>
