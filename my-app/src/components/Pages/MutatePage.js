...
function MutatePage({ isAuthenticated, token }) {
  ...
  function updateClip(clipmodel, checked){
    let clip_models = (job.clip_models)?job.clip_models:(job.discoart_tags)?job.discoart_tags.clip_models:[]
    let foundIndex = -1
      ...
      <Skeleton isLoaded={!loading}>
        ...
        <SimpleGrid columns={{sm: 2, md: 2}} spacing="20px">
          <FormControl>
            ...
            <Switch
              id="experimental"
              isDisabled
              isChecked = {job.experimental?job.experimental:false}
              onChange={(event) => {
            ...
            />
            {show_help && <FormHelperText>Opt-in for experimental new GPU agent mode.  Jobs may take longer to be taken, may fail, or might be better!</FormHelperText>}
          </FormControl>
          <FormControl>
        ...
        <FormControl>
          ...
          {show_help && <FormHelperText>
            In Disco Diffusion, text prompts can be a few words, a long sentence, or a few sentences. Writing prompts is an art in and of itself, browse around the site to get inspired!
          </FormHelperText>}
        </FormControl>
        ...
        <FormControl>
          ...
          {show_help && <FormHelperText>Proportion of cuts to grayscale.</FormHelperText>}
        </FormControl>
        <SimpleGrid columns={{sm: 1, md: 2}} spacing="20px">
        <FormControl>
        ...
        <FormControl>
          ...
          <Input value={job.transformation_percent || "[0.9]"} onChange={(event) => {
              let value = event.currentTarget.value
              let updatedJob = JSON.parse(JSON.stringify(job));
              updatedJob.transformation_percent = value;
              setJob({ ...job, ...updatedJob });
            }}/>
